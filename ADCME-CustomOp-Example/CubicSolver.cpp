...
class CubicSolverOp : public OpKernel {
private:
  
public:
  explicit CubicSolverOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {    
    ...
    // TODO:
    CubicSolver_forward(y_tensor, x_tensor, n);

  }
};
REGISTER_KERNEL_BUILDER(Name("CubicSolver").Device(DEVICE_CPU), CubicSolverOp);



class CubicSolverGradOp : public OpKernel {
private:
  
public:
  explicit CubicSolverGradOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    // TODO:
    int n = x_shape.dim_size(0);
    CubicSolver_backward(grad_x_tensor, grad_y_tensor, y_tensor, n);
    
  }
};
REGISTER_KERNEL_BUILDER(Name("CubicSolverGrad").Device(DEVICE_CPU), CubicSolverGradOp);
